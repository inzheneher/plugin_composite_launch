...
public class CompositeLaunchConfigurationTab extends AbstractLaunchConfigurationTab {
	...
	public static final String COMP_TAB_CONFIGURATIONS_LIST_NAME = "Select needed configurations:";
	...
	public void createControl(Composite parent) {
		Composite comp = new Composite(parent, SWT.MULTI | SWT.BORDER);
		...
		configurationViewer.getTree().setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
		...
		configurationViewer.addCheckStateListener(new ICheckStateListener() {
			public void checkStateChanged(CheckStateChangedEvent event) {
				((ILaunchConfigurationTreeNode) event.getElement()).setChecked(event.getChecked()); //enable  checkbox for checks
				((CheckboxTreeViewer) event.getCheckable()).refresh(true);//to mark parent's checkbox if on of child is checked
				CompositeLaunchConfigurationTab.this.updateLaunchConfigurationDialog(); // to enable refresh of Apply and Revert button
			}
		});
